...
?>
    ...
    public function render_cards($limit = 8, $category = '')
    {
        ...
        // Filter by _room_number meta key
        if ($category !== '') {
        ...
        while ($loop->have_posts()) : $loop->the_post();
            ...
            $bathroom_number = get_post_meta($id, '_bathroom_number', true);
            $permalink      = get_permalink($id);
        ?>
            <div class="appt-room-card-wrap">
                <a href="<?= esc_url($permalink) ?>" class="appt-room-card">
                    ...
                    <div class="appt-room-info">
                        ...
                        <div class="appt-room-top-row">
                            <span class="appt-room-name"><?= esc_html($title) ?></span>
                            <?php if ($rating) : ?>
                                <span class="appt-room-rating">
                                    <span class="appt-star">★</span><?= esc_html(number_format((float)$rating, 1)) ?>
                                </span>
                            <?php endif; ?>
                        </div>
                        ...
                        <!-- Location -->
                        <?php if ($location) : ?>
                            <span class="appt-room-location">📍 <?= esc_html($location) ?></span>
                        <?php endif; ?>

                        <!-- Rooms & Bathrooms -->
                        <div class="appt-room-features">
                            <?php if ($room_number) : ?>
                                <span class="appt-feat-item">🛏 <?= esc_html($room_number) ?>
                                    Room<?= intval($room_number) > 1 ? 's' : '' ?></span>
                            <?php endif; ?>
                            <?php if ($bathroom_number) : ?>
                                <span class="appt-feat-item">🚿 <?= esc_html($bathroom_number) ?>
                                    Bath<?= intval($bathroom_number) > 1 ? 'rooms' : 'room' ?></span>
                            <?php else : ?>
                                <span class="appt-feat-item">🚿 1 Bathroom</span>
                            <?php endif; ?>
                        </div>

                        <!-- Price -->
                        <span class="appt-room-price">
                            <strong>$<?= esc_html(number_format((float)$price, 2)) ?></strong> / Night
                        </span>
                ...
                <!-- Read More Button -->
                <div class="appt-card-readmore-wrap">
                    <a href="<?= esc_url($permalink) ?>" class="appt-readmore-btn">Read More</a>
                </div>
            </div>
<?php
        endwhile;
