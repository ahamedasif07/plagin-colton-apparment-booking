...
class Appartali_CPT {
    ...
    public function render_details( $post ) {
        ...
        /* Room type */
        $rt = get_post_meta( $post->ID, '_room_type', true );
        echo '<tr><th>Room Type</th><td><select name="room_type">';
        foreach ( [ 'apartment'=>'Apartment','studio'=>'Studio','villa'=>'Villa','house'=>'House','cottage'=>'Cottage' ] as $v=>$l ) {
            echo "<option value='$v'" . selected($rt,$v,false) . ">$l</option>";
        }
    ...
    public function render_gallery( $post ) {
        ...
        jQuery(function($){
            ...
            $('#appt_add_gallery').on('click',function(){
                ...
                frame=wp.media({title:'Select Gallery Images',multiple:true});
                frame.on('select',function(){
                    ...
                    att.forEach(function(a){
                        if(ids.indexOf(String(a.id))===-1){
                            ids.push(a.id);
                            $('#appt-gallery-wrap').append('<div class="appt-gal-item" style="position:relative;"><img src="'+a.sizes.thumbnail.url+'" style="width:90px;height:70px;object-fit:cover;border-radius:4px;"><button type="button" class="appt-remove-img button" data-id="'+a.id+'" style="position:absolute;top:2px;right:2px;padding:0 4px;font-size:11px;line-height:18px;">✕</button></div>');
                        }
                    });
            ...
            $(document).on('click','.appt-remove-img',function(){
                var id=$(this).data('id');
                var ids=$('#appt_gallery_ids').val().split(',').filter(function(i){return String(i)!==String(id);});
                $('#appt_gallery_ids').val(ids.join(','));
    ...
    public function render_host( $post ) {
        ...
        jQuery(function($){
            $('#appt_host_img_btn').on('click',function(){
                var u=wp.media({title:'Select Host Image',multiple:false});
                u.on('select',function(){$('#host_img_field').val(u.state().get('selection').first().toJSON().url);});
                u.open();
            });
        });
        </script>
    ...
    public function save_meta( $post_id ) {
        ...
        $text_fields = [
            'price_per_night','location','room_id','rating','room_type',
            'max_guests','cleaning_fee','service_fee','gallery_images',
            'host_name','host_image','host_reviews','host_ratings','host_years',
            'host_response_rate','host_response_time','host_work',
            'host_language','host_lives','host_cohost_name',
        ];
