...
while ( have_posts() ) :
    ...
    $amenity_icons = [
        'lock_door'  => '🔒', 'wifi' => '📶', 'tv' => '📺', 'luggage' => '🧳',
        'fridge'     => '🧊', 'kitchen' => '🍳', 'workspace' => '💼', 'washer' => '🫧',
        'hair_dryer' => '💨', 'iron' => '👔', 'ac' => '❄️', 'parking' => '🚗',
        'pool'       => '🏊', 'gym' => '🏋️',
    ];
    $room_type_labels = [
        'apartment' => 'Apartment', 'studio' => 'Studio', 'villa' => 'Villa',
        ...
        <div class="appt-single-layout">
            ...
            <div class="appt-single-left">
                ...
                <div class="appt-main-image-wrap">
                    <?php if ( $main_img ): ?>
                        <img src="<?= esc_url($main_img) ?>" alt="<?= esc_attr($title) ?>" class="appt-main-img" id="apptMainImg">
                    <?php else: ?>
                ...
                <div class="appt-guest-fav-row">
                    <div class="appt-guest-fav-badge">
                        ...
                        <div class="appt-gf-text">
                            ...
                            <span class="appt-gf-sub">One of the most loved homes<br>on Appartali, according to guests</span>
                        </div>
                ...
                <div class="appt-room-details-row">
                    ...
                    <div class="appt-room-detail-grid">
                        <div class="appt-rd-item"><span class="appt-rd-icon">🏠</span><span>Room in a <?= esc_html($room_type_label) ?></span></div>
                        <?php if ($max_guests): ?>
                        <div class="appt-rd-item"><span class="appt-rd-icon">👥</span><span>Up to <?= esc_html($max_guests) ?> guests</span></div>
                        <?php endif; ?>
                        <div class="appt-rd-item"><span class="appt-rd-icon">🔑</span><span>Your own room in a home, plus access to shared spaces</span></div>
                        <?php if ($host_super === '1'): ?>
                        <div class="appt-rd-item"><span class="appt-rd-icon">⭐</span><span><?= esc_html($host_name) ?> is a superhost – an experienced, highly rated host</span></div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="appt-meet-host-section">
                    ...
                    <div class="appt-meet-host-card">
                        <div class="appt-mh-avatar-col">
                            <?php if ($host_img): ?>
                                <img src="<?= esc_url($host_img) ?>" alt="<?= esc_attr($host_name) ?>" class="appt-mh-avatar">
                            <?php endif; ?>
                            ...
                            <div class="appt-mh-stats">
                                ...
                                <?php if ($host_years):   ?><span><?= esc_html($host_years) ?><small>Yrs hosting</small></span><?php endif; ?>
                            </div>
                        </div>
                        <div class="appt-mh-details-col">
                            ...
                            <?php endif; ?>
                            <?php if ($host_cohost): ?>
                                <div class="appt-mh-detail-row"><span class="appt-mh-dl">👤 Co-Host:</span><span><?= esc_html($host_cohost) ?></span></div>
                            <?php endif; ?>
                            <div class="appt-mh-detail-title">Host Details</div>
                            <?php if ($host_rrate): ?>
                                <div class="appt-mh-detail-row"><span class="appt-mh-dl">📊 Response Rate:</span><span><?= esc_html($host_rrate) ?>%</span></div>
                            <?php endif; ?>
                            <?php if ($host_rtime): ?>
                                <div class="appt-mh-detail-row"><span class="appt-mh-dl">⏱ Response Time:</span><span><?= esc_html($host_rtime) ?></span></div>
                            <?php endif; ?>
                            <?php if ($host_work): ?>
                                <div class="appt-mh-detail-row"><span class="appt-mh-dl">💼 My work:</span><span><?= esc_html($host_work) ?></span></div>
                            <?php endif; ?>
                            <?php if ($host_lang): ?>
                                <div class="appt-mh-detail-row"><span class="appt-mh-dl">🗣 Language:</span><span><?= esc_html($host_lang) ?></span></div>
                            <?php endif; ?>
                            <?php if ($host_lives): ?>
                                <div class="appt-mh-detail-row"><span class="appt-mh-dl">📍 Lives in:</span><span><?= esc_html($host_lives) ?></span></div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="appt-single-right">
                <div class="appt-booking-widget" id="apptBookingWidget"
                     data-apartment-id="<?= intval($id) ?>"
                     data-price="<?= esc_attr($price) ?>"
                     data-cleaning-fee="<?= esc_attr($clean_fee) ?>"
                    ...
                    <div class="appt-bw-dates">
                        <div class="appt-bw-date-group">
                            ...
                            <input type="date" id="apptCheckin" name="checkin" class="appt-date-input" min="<?= date('Y-m-d') ?>">
                        </div>
                        <span class="appt-bw-arrow">→</span>
                        <div class="appt-bw-date-group">
                            <label>Check Out</label>
                            <input type="date" id="apptCheckout" name="checkout" class="appt-date-input" min="<?= date('Y-m-d', strtotime('+1 day')) ?>">
                        </div>
                    </div>
                    ...
                    <div class="appt-bw-breakdown" id="apptBreakdown" style="display:none;">
                        <div class="appt-bw-line"><span id="apptNightsLabel">$<?= number_format($price,2) ?> × 0 nights</span><span id="apptSubtotal">$0.00</span></div>
                        <div class="appt-bw-line"><span>Cleaning fee</span><span>$<?= number_format($clean_fee,2) ?></span></div>
                        <div class="appt-bw-line"><span>Appartali service fee</span><span>$<?= number_format($svc_fee,2) ?></span></div>
...
<!-- ══════════════════════════════════
     BOOKING POPUP MODAL
══════════════════════════════════ -->
<div id="apptBookingModal" class="appt-popup-overlay" style="display:none;" role="dialog" aria-modal="true" aria-label="Booking Form">
    <div class="appt-popup-box">
        ...
        <div class="appt-popup-form">
            ...
            <div class="appt-form-group appt-form-full">
                ...
                <textarea id="apptSpecialReq" rows="3" placeholder="Any special requests or notes for the host..."></textarea>
            </div>
        </div>
...
<div id="apptSuccessModal" class="appt-popup-overlay" style="display:none;">
    <div class="appt-popup-box appt-success-box">
        ...
        <p>Thank you! Your booking is currently <strong>pending</strong> review. You will receive a confirmation email at <strong id="apptSuccessEmail"></strong> once it's approved.</p>
        <button class="appt-reserve-btn" onclick="document.getElementById('apptSuccessModal').style.display='none'">Done</button>
    </div>
</div>
