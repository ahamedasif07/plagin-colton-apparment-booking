...
(function ($) {
  ...
  function isDateBlocked(dateStr) {
    return (
      window.apptBlockedDates && window.apptBlockedDates.indexOf(dateStr) !== -1
    );
  }

  function hasBlockedDatesInRange(checkin, checkout) {
    if (!window.apptBlockedDates || !window.apptBlockedDates.length)
      return false;
  ...
  function checkAvailability(checkin, checkout) {
    ...
    $availMsg
      ...
      .html('<span style="color:#aaa">Checking availability…</span>')
      .show();
  ...
  function updateBreakdown(nights, price, clean, svc, total) {
    $("#apptNightsLabel").text(
      "$" + fmt(price) + " × " + nights + " night" + (nights === 1 ? "" : "s"),
    );
  ...
  function openBookingPopup(ci, co) {
    ...
    $("#apptPopupNights").text(
      currentBookingData.nights +
        " night" +
        (currentBookingData.nights === 1 ? "" : "s"),
    );
  ...
  $(document).on("click", "#apptPopupClose, .appt-popup-overlay", function (e) {
    if (
      $(e.target).is(".appt-popup-overlay") ||
      $(e.target).is("#apptPopupClose")
    ) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.success) {
          ...
          // Reset form
          $(
            "#apptGuestName, #apptGuestEmail, #apptGuestPhone, #apptSpecialReq",
          ).val("");
          $checkin.val("");
          $checkout.val("");
          hideBreakdown();
  ...
  function showPopupError(msg) {
    ...
    // scroll to error
    $("#apptPopupError")[0].scrollIntoView({
      behavior: "smooth",
      block: "nearest",
    });
  }
...
// popupop scroll
function openPopup() {
  const popup = document.querySelector(".appt-popup-box");

  document.body.style.overflow = "hidden"; // background scroll off
